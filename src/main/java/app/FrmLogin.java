...
public class FrmLogin extends JFrame {
	...
	void registrar() {
		...
		// validacion
		//select*from tb_xx where usr = ? and clav=?
		
		EntityManagerFactory fabrica = Persistence.createEntityManagerFactory("jpa_sesion04");
		EntityManager em = fabrica.createEntityManager();
		
		try {
			Usuario u = em.createQuery("Select u from Usuario u where u.correo = :xmail and u.clave = :xclave",Usuario.class).
					setParameter("xmail", usuario).setParameter("xclave", clave).getSingleResult();
